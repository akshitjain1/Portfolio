...
export default function CodeProfiles() {
  ...
  // Fetch GitHub Stats with optimized error handling
  const fetchGitHubStats = useCallback(async () => {
    try {
      const [userResponse, reposResponse, contributionsResponse] = await Promise.allSettled([
        fetch('https://api.github.com/users/akshitjain1'),
        fetch('https://api.github.com/users/akshitjain1/repos?per_page=100'),
        fetch('https://github-contributions-api.jogruber.de/v4/akshitjain1')
      ])

      let userData, reposData, contributionsData
      
      if (userResponse.status === 'fulfilled') {
        userData = await userResponse.value.json()
      }
      if (reposResponse.status === 'fulfilled') {
        reposData = await reposResponse.value.json()
      }
      if (contributionsResponse.status === 'fulfilled') {
        contributionsData = await contributionsResponse.value.json()
      }

      const totalStars = Array.isArray(reposData) ? 
        reposData.reduce((acc: number, repo: any) => acc + (repo.stargazers_count || 0), 0) : 0
  ...
  // Fetch LeetCode Stats with optimized error handling
  ...
  // Fetch GeeksforGeeks Stats (static data with future API support)
  const fetchGfgStats = useCallback(async () => {
    try {
      // GFG doesn't have a public API, using static data
      // This structure allows for easy future API integration
      const staticData = {
  ...
  // Optimized fetch all stats function
  ...
  }, [fetchGitHubStats, fetchLeetCodeStats, fetchGfgStats])

  useEffect(() => {
    if (inView && !githubStats && !leetcodeStats && !gfgStats) {
      fetchAllStats()
    }
  }, [inView, githubStats, leetcodeStats, gfgStats, fetchAllStats])
  ...
  const summaryStats = useMemo(() => ({
    totalProblemsAndRepos: (githubStats?.totalRepos || 0) + (leetcodeStats?.totalSolved || 0) + (gfgStats?.totalProblemsSolved || 0),
    githubContributions: githubStats?.totalContributions || 0,
    leetcodeSolved: leetcodeStats?.totalSolved || 0,
    gfgScore: gfgStats?.overallScore || 0,
  }), [githubStats, leetcodeStats, gfgStats])
          ...
          >
            <div className="bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 rounded-xl p-6 text-center">
              <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">
                {isLoading ? '...' : summaryStats.totalProblemsAndRepos}
              </div>
            ...
            <div className="bg-gradient-to-r from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 rounded-xl p-6 text-center">
              <div className="text-2xl font-bold text-green-600 dark:text-green-400">
                {isLoading ? '...' : summaryStats.githubContributions}
              </div>
            ...
            <div className="bg-gradient-to-r from-yellow-50 to-yellow-100 dark:from-yellow-900/20 dark:to-yellow-800/20 rounded-xl p-6 text-center">
              <div className="text-2xl font-bold text-yellow-600 dark:text-yellow-400">
                {isLoading ? '...' : summaryStats.leetcodeSolved}
              </div>
            ...
            <div className="bg-gradient-to-r from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 rounded-xl p-6 text-center">
              <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">
                {isLoading ? '...' : summaryStats.gfgScore}
              </div>
